...
CREATE INDEX IF NOT EXISTS idx_sessions_active ON public.sessions(user_id, revoked, expires_at);
...
CREATE TABLE IF NOT EXISTS public.user_dashboard_layout (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,
  layout_data JSONB NOT NULL DEFAULT '{"accountant": {"iconOrder": [], "viewMode": "grid", "widgets": []}}'::jsonb,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_security_audit_action ON public.security_audit_logs(action, created_at DESC);
...
DROP POLICY IF EXISTS "Users can insert their own dashboard layout" ON public.user_dashboard_layout;
...
DROP TRIGGER IF EXISTS update_user_dashboard_layout_updated_at ON public.user_dashboard_layout;
